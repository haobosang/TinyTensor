    ...
    std::vector<std::shared_ptr<Tensor<float>>> &outputs)
{
    ...
    for (uint32_t i = 0; i < batch; ++i)
    {
        ...
        CHECK(output_data->rows() == output_h_ && output_data->cols() == output_w_ && output_data->channels() == input_c) << "The output size of adaptive pooling is error";
...
ParseParameterAttrStatus AdaptiveAveragePoolingLayer::GetInstance(const std::shared_ptr<RuntimeOperator> &op, std::shared_ptr<Layer> &avg_layer)
{
